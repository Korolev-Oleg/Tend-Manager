...
from interface.UI import UI_main as UiRight
from interface.UI import UI_mainLeft as UiLeft
from interface.UI.RESOURSE import resource_path
from interface.UX.TabGeneral import GeneralTab
...
class MainUi(QtWidgets.QMainWindow, UiRight.Ui_Ui, UiLeft.Ui_Ui):
    def __init__(self, restoredData, localRestored):
        ...
        self._lineAppSecurity.currentIndexChanged.connect(
            self._calculating_percent_fields)

        self._lineContractSecurity.currentIndexChanged.connect(
            self._calculating_percent_fields)

        self._lineCurrentPrice.textChanged.connect(
            self._calculating_percent_fields)
    ...
    def _calculating_percent_fields(self):
        """
        Подсчет процентов в полях:
        Обеспечение контракта и обеспечение заявки
        относительно НМЦК
        :return: None
        """

        sec_application = get_cash(self._lineAppSecurity)
        sec_contract = get_cash(self._lineContractSecurity)

        try:
            nmck = self._lineCurrentPrice.text().replace(' ', '')
            nmck = int(nmck) if isinstance(nmck, int) else float(nmck)

            if nmck:
                percent = (nmck / 100) * 5
                price = f"{percent:.{2}f}"
                if sec_application == '5%':
                    self._lineAppSecurity.setCurrentText(price)
    ...
    def start_validator(self):
        ...
        self.hide()
        subprocess.run([r"C:\Users\Huston\Documents\GitHub\Tend-Manager\validator\au3\validator.exe", last_app_path])
        self.show()
        # TODO Додолать валидатор (Работает версия для презентации диплома!)
        return 0
        ...
        if file:
            ...
            data = file, Validator
            self.progress = Progress(data, validator=True)
    ...
    def eventFilter(self, obj, event):
        """ Глобальный Фильтр событий окна"""
