...
def init(variables, form):
    ...
    df[6]['value'] = f.getStrCash(str(form['appSecurity']))     # ^прописью
    df[7]['value'] = form['contractSecurity']                   # Обесп контр
    df[8]['value'] = f.getStrCash(str(form['contractSecurity']))# ^прописью
    df[9]['value'] = form['currentPrice']                       # НМЦК
    df[10]['value'] = f.getStrCash(str(form['currentPrice']))   # ^прописью
